package Data::ObjectDriver::Role::Object;
...
sub driver {
    my $class = shift;
    $class->meta->driver; #||= $class->properties->{get_driver}->();
}

sub get_driver {
    my $class = shift;
    $class->meta->driver; #get_driver} = shift if @_;
}
...
sub is_pkless {
    my $obj = shift;
    my $prop_pk = $obj->meta->primary_key;
    return 1 if ! $prop_pk;
...
sub primary_key_tuple {
    my $obj = shift;
    my $pk = $obj->meta->primary_key || return;
...
sub datasource { $_[0]->meta->datasource }
...
sub set_values_internal {
    my $obj = shift;
    my $values = shift;
    ## XXX disable is_changed registration here.
    for my $col (keys %$values) {
...
sub has_column {
    #lame
    my $obj = shift;
    my %columns = map { $_ => 1 } @{ $obj->meta->columns };
...
sub column_values { 
    my $obj = shift;
    my $column_values = {};
    for my $column (@{ $obj->meta->columns || []}) {
        $column_values->{ $column } = $obj->$column();
    }
    return $column_values;
}

sub column {
    die "column is deprecated";
}
